...
//		if(empty($buttons))
//			return;
		echo '<span class="page_sum">';
		//echo $this->ItemCount.'条&nbsp;&nbsp;';	
		echo $this->header;
		echo '</span>';
	...
	protected function createPageButtons()
	{
		...
		// page list
		if($this->pagelist){
			$html='<select style="cursor: pointer;min-width:45px" onChange="javascript:var select_v=$(this).val();window.location.href=\''.$this->createPageUrl('').'/page/\'+select_v"  href="'.$this->createPageUrl('').'">';
			for($i=0;$i<$pageCount;++$i){
				$html.='<option ';
				if($currentPage==$i){
					$html.='selected';
				}
				$html.=' value="'.($i+1).'">'. ($i+1) .'</option>';
			}
			$html.='</select>';
			$buttons[]=$html;
		}
